...
class AntiviralGNN(nn.Module):
    ...
    def __init__(self, num_node_features=NUM_ATOM_FEATURES, num_edge_features=NUM_BOND_FEATURES):
        ...
        # Using both mean and max pooling -> hidden_dim * 2 features
...
def train():
    ...
    with open("results_gnn.tsv", "a") as f:
        if os.path.getsize("results_gnn.tsv") == 0 if os.path.exists("results_gnn.tsv") else True:
            f.write("timestamp\tval_auc\tnum_params\tdescription\n")
        f.write(f"{datetime.now().isoformat()}\t{best_auc:.4f}\t{num_params}\t"
                f"GCN 3-layer 128-dim mean+max pool\n")
